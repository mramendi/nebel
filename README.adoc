...
|`-n`
|Process using the new (2025) template. `context` definitions are added to all assemblies and `_{context}` is added to all `id` values, this results in the top level assemply `id` including the literal `_{context}` so you must fix this in postprocessing. Also supports attribute definitions at the start of assemblies/modules (before the title) and creates a `links.csv` file with the new link for every original ID.
